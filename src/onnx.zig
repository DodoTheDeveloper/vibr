...
pub fn run(allocator: std.mem.Allocator, model_path: [:0]const u8, prompt: []const i64) !void {
    ...
    const seq_len: i64 = 5;
    //var input_ids: [5]OrtValueBuffer = .{ .{ .I64 = 1234 }, .{ .I64 = 5678 }, .{ .I64 = 9012 }, .{ .I64 = 3456 }, .{ .I64 = 7890 } };
    //
    ...
    for (model_info.*.inputs, 0..) |input, idx| {
        ...
        const name_null_terminated = try allocator.alloc(u8, input.name.len+1);
        std.mem.copyForwards(u8, name_null_terminated[0..input.name.len], input.name[0..]);
        name_null_terminated[name_null_terminated.len-1] = 0;
        input_name_ptrs[idx] = name_null_terminated.ptr;
    ...
    for (model_info.*.outputs, 0..) |output, idx| {
        // fill output_buffers with buffers
        //

        const name_null_terminated = try allocator.alloc(u8, output.name.len+1);
        std.mem.copyForwards(u8, name_null_terminated[0..output.name.len], output.name[0..]);
        name_null_terminated[name_null_terminated.len-1] = 0;
        output_name_ptrs[idx] = name_null_terminated.ptr;
