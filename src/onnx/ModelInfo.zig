...
pub const ModelInfo = struct {
    pub const Input = struct { name: []const u8, data_type: onnx.ONNXTensorElementDataType, dimensions: []i64, is_cached: bool };
    pub const Output = struct { name: []const u8, data_type: onnx.ONNXTensorElementDataType, dimensions: []i64, is_cached: bool };
...
test "ModelInfo create & destroy" {
    ...
    given_inputs[0] = .{ .name = try given_allocator.dupe(u8, "input_0"), .data_type = 0, .dimensions = try given_allocator.dupe(i64, &.{ 1, 2, 3 }), .is_cached = false };
    given_inputs[1] = .{ .name = try given_allocator.dupe(u8, "past_key_values_1"), .data_type = 0, .dimensions = try given_allocator.dupe(i64, &.{ 1, 2, 3 }), .is_cached = true };

    const given_outputs = try given_allocator.alloc(ModelInfo.Output, 2);
    given_outputs[0] = .{ .name = try given_allocator.dupe(u8, "output_0"), .data_type = onnx.ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64, .dimensions = try given_allocator.dupe(i64, &.{ 1, 2, 3 }), .is_cached = false };
    given_outputs[1] = .{ .name = try given_allocator.dupe(u8, "past_key_values_1"), .data_type = onnx.ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64, .dimensions = try given_allocator.dupe(i64, &.{ 1, 2, 3 }), .is_cached = true };
