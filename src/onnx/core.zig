...
pub fn CreateEnv(api: *const onnx.OrtApi, log_level: onnx.OrtLoggingLevel, log_id: []const u8, out: **onnx.OrtEnv) !void {
    var maybe_env_ptr: ?*onnx.OrtEnv = null;
    const create_env_fn = api.*.CreateEnv orelse return OnnxError.FnDoesntExist;
    try checkStatus(create_env_fn(log_level, log_id.ptr, &maybe_env_ptr), api);
...
pub fn SessionGetOutputCount(api: *const onnx.OrtApi, session: *const onnx.OrtSession, out: *usize) !void {
    const func = api.*.SessionGetOutputCount orelse return OnnxError.FnDoesntExist;
    return try checkStatus(func(session, out), api);
}

pub fn SessionGetOutputName(api: *const onnx.OrtApi, session: *const onnx.OrtSession, index: usize, ort_allocator: *onnx.OrtAllocator, value: *[*:0]u8) !void {
